#!/usr/bin/env python3
"""
import_event.py - Import Ghidra-decompiled functions into colosseum_event.c stubs.

Only replaces register-variable pseudo-code stubs (u32 r3 = 0; pattern).
Preserves functions that already have real C code.
"""
...
SRC_FILE = PROJECT_ROOT / "src" / "game" / "colosseum_event.c"
...
def is_stub(source_text, func_name):
    """Check if a function is a register-variable stub."""
    rng = find_function_range(source_text, func_name)
    if not rng:
        return False
    body = source_text[rng[0]:rng[1]]
    return 'u32 r3 = 0;' in body or ('u32 r0 = 0;' in body and 'u32 r31 = 0;' in body)
...
def phase1_fix_declarations(source, ghidra_dict=None):
    ...
    # Remove forward declarations for functions in our range
    lines = source.split('\n')
    new_lines = []
    ...
    # Change function definitions to () params with local var decls
...
def main():
    print("=" * 70)
    print("COMPREHENSIVE IMPORT: colosseum_event.c")
    print("=" * 70)
    ...
    # Backup
    backup = str(SRC_FILE) + '.event_import.bak'
    ...
    ok, out = run_compile()
    if not ok:
        print("  Phase 1 compile check (pre-existing issues ok, continuing)")

    # Identify stubs
    source = SRC_FILE.read_text()
    stub_fns = []
    for fn_name in sorted(ghidra_dict.keys(), key=lambda n: int(n[3:], 16)):
        if is_stub(source, fn_name):
            stub_fns.append(fn_name)

    print(f"\nStub functions to import: {len(stub_fns)}")
    print(f"Non-stub functions (preserved): {len(ghidra_dict) - len(stub_fns)}")

    # Phase 2: Import stub functions one by one
    imported = 0
    failed_list = []
    skipped = 0

    print(f"\nPhase 2: Importing {len(stub_fns)} stub functions...")

    for i, fn_name in enumerate(stub_fns):
        ...
        if ok:
            imported += 1
            if (imported % 10) == 0:
                print(f"  [{i+1}/{len(stub_fns)}] {fn_name}: OK ({imported} imported)")
        else:
            SRC_FILE.write_text(source)
            err = ''
            ...
            if (i + 1) % 20 == 0:
                print(f"  [{i+1}/{len(stub_fns)}] progress... ({imported} ok, {len(failed_list)} failed)")
    ...
    print(f"{'='*70}")
    print(f"  Total stubs: {len(stub_fns)}")
