/**
 * @file gapp_range_80101B34.c
 * @brief gapp/GSgapp -- per-frame vsync/background callback dispatch and
 *        GX load-meter (Colosseum "menu" UI subsystem support). Split
 *        from the gs_model.c splitter bucket (address range
 *        0x80101B34 - 0x80102004, 6 fns). Structural match only (no
 *        shared symbol names) against XD's gapp TU
 *        (0x8010BD24-0x8010C320, gappBackgroundCallback/
 *        gappVSyncCallback/GSgapp*); kept as a range-named file.
 */
